...
namespace Engine
{
    ...
    struct AuthContext {
        ...
        uint32_t userId = 0;   ///> Unique identifier for the user
        uint64_t token = 0;    ///> Authentication token
        uint32_t ttlSec = 0;   ///> Time-to-live for the authentication token in seconds
