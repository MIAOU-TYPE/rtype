...
namespace Engine
{
    ...
    {
        if (_entityMap.find(data.id) != _entityMap.end()) {
            std::cerr << "[ClientWorld::applyCreate] Entity with ID " << data.id
        ...
        _entityMap.emplace(entity, data.id);
    ...
    {
        Ecs::Entity entity = static_cast<Ecs::Entity>(data.id);
        auto it = _entityMap.find(data.id);
    ...
    {
        auto it = _entityMap.find(entity.id);
        if (it == _entityMap.end()) {
            applyCreate(EntityCreate{entity.id, entity.x, entity.y, entity.sprite});
            return;
        }
        Ecs::Entity localEntity = it->second;
        _registry.emplaceComponent<Position>(localEntity, Position{entity.x, entity.y});
        _registry.emplaceComponent<Drawable>(localEntity, Drawable{entity.sprite});
    }
