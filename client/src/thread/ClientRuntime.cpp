...
namespace Thread
{
    ...
    void ClientRuntime::runDisplay()
    {
        ...
        while (_running && _stateManager->isRunning()) {
            ...
            if (_pendingHome.exchange(false, std::memory_order_acq_rel)) {
                try {
    ...
    void ClientRuntime::setupGlobalEventHandlers()
    {
        ...
        _eventBus->on<Engine::CreateRoomRequested>([this](const Engine::CreateRoomRequested &e) {
            ...
            _tcpClient->sendPacket(
                *_tcpPacketFactory.makeCreateRoom(req, e.roomName, e.maxPlayers, e.difficulty, e.gameMode, e.levelPath));
        });
