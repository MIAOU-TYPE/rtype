...
namespace Game
{
    void PowerUpBarSystem::update(IGameWorld &world, const float)
    {
        ...
        reg.view<Ecs::PlayerPowerUp>(
            [&](const Ecs::Entity playerEntity, Ecs::PlayerPowerUp &powerUp) {
                playersWithPowerUp.emplace_back(playerEntity, &powerUp);
            });

        std::sort(playersWithPowerUp.begin(), playersWithPowerUp.end(),
            [](const auto &a, const auto &b) { return static_cast<size_t>(a.first) < static_cast<size_t>(b.first); });
