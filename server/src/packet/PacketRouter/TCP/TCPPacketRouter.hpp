    ...
    class TCPPacketRouter {
      public:
        TCPPacketRouter(std::shared_ptr<Server::ISessionManager> sessions, std::shared_ptr<Engine::RoomManager> rooms,
            std::shared_ptr<Server::IServer> tcpServer, std::shared_ptr<Factory::TCPPacketFactory> out)
            : _sessions(std::move(sessions)), _rooms(std::move(rooms)), _tcp(std::move(tcpServer)),
              _packetFactory(std::move(out))
        {
        }
        ...
        void handle(const std::shared_ptr<IPacket> &pkt) const
        {
            ...
            switch (h.type) {
                case Protocol::TCP::HELLO:
                    onHello(*addr, sessionId, h.requestId, r);
                    break;
                    // case Protocol::TCP::LOGIN:      onLogin(*addr, sessionId, h.requestId, r); break;
                ...
                case Protocol::TCP::LEAVE_ROOM:
                    onLeaveRoom(*addr, sessionId, h.requestId);
                    break;
                    // case Protocol::TCP::READY: onReady(*addr, sessionId, h.requestId, r); break;
                case Protocol::TCP::START_GAME:
                    onStartGame(*addr, sessionId);
                    break;
                    // case Protocol::TCP::DISCONNECT: onDisconnect(*addr, sessionId, h.requestId); break;
      ...
      private:
        ...
        void onHello(const sockaddr_in &addr, const int sessionId, const uint32_t req, TCP::Reader &r) const
        {
            uint16_t ver = 0;
            uint16_t udpPort = 0;
            try {
        ...
        void onListRooms(const sockaddr_in &addr, const uint32_t req) const
        {
            TCP::Writer b;
        ...
        void onCreateRoom(const sockaddr_in &addr, const uint32_t req, TCP::Reader &r) const
        {
            std::string roomName;
        ...
        void onJoinRoom(const sockaddr_in &addr, const int sessionId, const uint32_t req, TCP::Reader &r) const
        {
            uint32_t roomId = 0;
        ...
        void onStartGame(const sockaddr_in &addr, const int sessionId) const
        {
        ...
        std::shared_ptr<Server::ISessionManager> _sessions = nullptr;
        std::shared_ptr<Engine::RoomManager> _rooms = nullptr;
        std::shared_ptr<Server::IServer> _tcp = nullptr;
        std::shared_ptr<Factory::TCPPacketFactory> _packetFactory = nullptr;
    };
} // namespace Net