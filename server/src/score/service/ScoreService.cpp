...
namespace Engine
{
    ...
    void ScoreService::saveScore(const std::string& username, const int score) const
    {
        ...
        if (sqlite3_step(st.raw()) != SQLITE_DONE)
            throw ScoreServiceError("{ScoreService::saveScore} step failed: " + std::string(sqlite3_errmsg(_db->raw())));
    }
    ...
    std::vector<ScoreEntry> ScoreService::getTopScores(const std::size_t limit) const
    {
        ...
        while (true) {
            ...
            if (rc == SQLITE_DONE)
                break;
            if (rc != SQLITE_ROW)
                throw ScoreServiceError("{ScoreService::getTopScores} step failed: " + std::string(sqlite3_errmsg(_db->raw())));
