...
namespace Net
{
    ...
    {
        return ::setsockopt(s, level, optName, static_cast<const char *>(optVal), static_cast<socklen_t>(optLen));
    }
    ...
    int NetWrapper::setSocketOpt(socketHandle s, int level, int optName, const void *optVal, int optLen)
    {
        return ::setsockopt(s, level, optName, const_cast<const char *>(static_cast<const char *>(optVal)),
